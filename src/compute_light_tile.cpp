...
LightTileResources CreateLightTileResources(uint32_t width, uint32_t height) {
  ...
  resources.tile_count_x = (width + kTileSize - 1) / kTileSize;
  resources.tile_count_y = (height + kTileSize - 1) / kTileSize;

  uint32_t total_tiles = resources.tile_count_x * resources.tile_count_y;
  ...
  resources.tile_light_index_ssbo = CreateSSBO(nullptr, total_size);

  // Create debug heatmap texture (RGBA8).
  glCreateTextures(GL_TEXTURE_2D, 1, &resources.debug_heatmap_texture);
  glTextureStorage2D(resources.debug_heatmap_texture, 1, GL_RGBA8, width,
                     height);

  LOG(INFO) << "Created light tile resources: " << resources.tile_count_x << "x"
            << resources.tile_count_y << " tiles (" << total_tiles
...
void DestroyLightTileResources(LightTileResources& resources) {
  if (resources.tile_light_index_ssbo.id != 0) {
    DestroySSBO(resources.tile_light_index_ssbo);
    resources.tile_light_index_ssbo = {};
  }
  if (resources.debug_heatmap_texture != 0) {
    glDeleteTextures(1, &resources.debug_heatmap_texture);
    resources.debug_heatmap_texture = 0;
  }
}
...
bool ResizeLightTileResources(LightTileResources& resources, uint32_t width,
  ...
  DestroyLightTileResources(resources);
  resources = CreateLightTileResources(width, height);
  return true;
}
...
void ComputeTileLightList(const Camera& camera, const RenderTarget& hdr_target,
  ...
  // Resize if necessary.
  ResizeLightTileResources(resources, hdr_target.width, hdr_target.height);
  ...
  BindSSBO(resources.tile_light_index_ssbo, 2);

  // Bind depth texture.
  glBindTextureUnit(15, hdr_target.depth_buffer);

  // Bind debug heatmap as image.
  glBindImageTexture(14, resources.debug_heatmap_texture, 0, GL_FALSE, 0,
  ...
  // Dispatch.
  glDispatchCompute(resources.tile_count_x, resources.tile_count_y, 1);
